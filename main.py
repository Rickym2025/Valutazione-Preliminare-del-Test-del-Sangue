...
# ==================================================
import streamlit as st  # <--- IMPORT FONDAMENTALE!
...
try:
    # Usiamo st.secrets perché 'st' è stato importato
...
def main():
    ...
    # ORA POSSIAMO USARE 'st' PERCHÉ È STATO IMPORTATO PRIMA
        ...
        if current_file_id != st.session_state.processed_file_id:
            ...
            with analysis_placeholder.container():
             ...
             with analysis_placeholder.container():
                 if tipo_file == "Immagine (JPG, PNG)":
                     try:
                         # Riprova a mostrare l'immagine se il file caricato è ancora disponibile
                         if file_caricato:
                            immagine = Image.open(file_caricato)
                            st.image(immagine, caption="Anteprima Referto (già analizzato)", use_container_width=True)
                            st.markdown("---")
                     except: pass # Ignora errori qui, è solo anteprima
...
# La chiamata finale DEVE essere l'ultima cosa nel file (a parte commenti)
